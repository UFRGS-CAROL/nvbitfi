...
extern "C" __device__ __noinline__ void inject_error(uint64_t piinfo,
	...
	unsigned long long currCounter1 = atomicAdd(
			(unsigned long long *) &counters[NUM_ISA_INSTRUCTIONS + grp_index],
			1);
	unsigned long long currCounter2 = atomicAdd(
			(unsigned long long *) &counters[NUM_COUNTERS - 2],
			(grp_index != G_NODEST));
	unsigned long long currCounter3 = atomicAdd(
			(unsigned long long *) &counters[NUM_COUNTERS - 1],
				...
				if (DUMMY) { // no error is injected
					...
					switch (igid) {
						...
						case G_FP64: { // inject into one of the regs written by the inst
							...
							inj_info->afterVal = beforeValFloat * defined_fp32_relative_error;
							break;
						}
